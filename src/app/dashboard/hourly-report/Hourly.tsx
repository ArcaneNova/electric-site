...
function HourlyMultiModel() {
  ...
  const adjustHourlyDemand = (hour: number, demand: number): number => {
    ...
    const adjustmentPercentage = adjustments[hour] || 0;
    const adjustment = (demand * adjustmentPercentage) / 100;
    return demand + adjustment;
  };
    ...
    try {
      ...
      for (const modelType of modelTypes) {
        ...
        for (let hour = 0; hour < 24; hour++) {
          ...
          let demand = await sendToMLModel(formattedData, modelType);
          
          // Apply hourly adjustment
          demand = adjustHourlyDemand(hour, demand);
